...
// ユーザの登録した食材に紐づく栄養
struct UserNutrition : Codable, FetchableRecord, MutablePersistableRecord {
    ...
    static func create(_ db: Database) throws {
        try db.create(table: databaseTableName, body: { (t: TableDefinition) in
            t.column("userNutritionId", .double).primaryKey()
            t.column("userFoodId", .double).notNull()
